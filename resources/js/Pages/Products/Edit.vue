<template>
    <AuthenticatedLayout>
        <template #header>
            <h2 class="h4 mb-0">Modifier {{ product.name }}</h2>
        </template>
...
const form = reactive({
    ...
    unit_price: parseFloat(props.product.unit_price),
    cost_price: parseFloat(props.product.cost_price || 0),
    vat_rate: parseFloat(props.product.vat_rate),
...
const submit = () => {
    router.put(route('products.update', props.product.id), form);
};
</script>
